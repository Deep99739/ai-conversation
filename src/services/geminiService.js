...
export class GeminiService {
  ...
  // Analyze audio characteristics from transcript and duration
  static analyzeAudioCharacteristics(transcript, durationMs) {
    if (!transcript || durationMs <= 0) {
      return {
        speakingRate: 0,
        pauseCount: 0,
        energyLevel: EnergyLevel.MODERATE,
        confidenceLevel: ConfidenceLevel.NEUTRAL
      };
    }
    ...
    const speakingRate = Math.round(wordCount / durationMinutes) || 0;

    // Estimate pause count based on sentence structure
    const sentenceCount = (transcript.match(/[.!?]+/g) || []).length;
    const pauseCount = Math.max(0, sentenceCount - 1);

    // Determine energy and confidence levels based on speaking rate
    let energyLevel = EnergyLevel.MODERATE;
    let confidenceLevel = ConfidenceLevel.NEUTRAL;

    if (speakingRate < 100) {
      energyLevel = EnergyLevel.VERY_LOW;
      confidenceLevel = ConfidenceLevel.VERY_HESITANT;
    } else if (speakingRate < 150) {
      energyLevel = EnergyLevel.LOW;
      confidenceLevel = ConfidenceLevel.HESITANT;
    } else if (speakingRate < 200) {
      energyLevel = EnergyLevel.MODERATE;
      confidenceLevel = ConfidenceLevel.NEUTRAL;
    } else if (speakingRate < 250) {
